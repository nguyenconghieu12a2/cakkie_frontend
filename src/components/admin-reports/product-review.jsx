...
const api = "/api/admin/reports/product-review"
...
const ProductReview = ({startDate, endDate}) => {
  ...
  const fetchProductReviewReport = async() =>{
    try{
      const response = await axios.get(`${api}`);
      setProReview(response.data);
  ...
  const applyDateFilter = (data) => {
    ...
    setFilteredProductReview(filteredData);
  };

  useEffect(() => {
    fetchProductReviewReport();
  }, []);

  useEffect(() => {
    if (startDate || endDate) {
      applyDateFilter(proReview);

      //Reset the current page to 1 when data is filtered
      setCurrentPage(1);
    }
  }, [startDate, endDate, proReview, itemPerPage]);

  const totalResult = filteredProductReview.length;
  const totalPages = Math.ceil(totalResult / itemPerPage);

  const pagedResult = filteredProductReview.slice(
  ...
  return (
    <div className="card-body">
      ...
      <table className="table table-striped">
        ...
        {proReview.length === 0 || filteredProductReview.length === 0 ? (
          <tr>
            <td colSpan="7" style={{ textAlign: "center", padding: "20px" }}>
              <div className="no-data-message">No Product reviews found.</div>
            </td>
          </tr>
        ) : (
        pagedResult.map((odr, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td>{odr.penddingReviews}</td>
                <td>{odr.approvedReviews}</td>
                <td>{odr.rejectedReviews}</td>
                <td>{odr.total}</td>
              </tr>
            );
          }))}
